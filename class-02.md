...
so to move to a new line we use this.
...
* CSS treats each HTML element as if it appears inside  X its own box and uses rules to indicate 
how that element should look.

* Rules are made up of selectors (that specify the  X elements the rule applies to) and 
...
* comments :w euse it  to explain what your code does. They help make your code easier to read and understand. 
...
4. CHANGING VALUES IN AN ARRAY (updating) : ( ex  the third item on the list is changed from 'custom' to 'beige' like
 this colors[2] = 'beige' ; )
------------------------------------------------------------------------------------------------------------------------------
